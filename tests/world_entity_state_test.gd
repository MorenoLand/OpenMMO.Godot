...
func _init() -> void:
	...
	var npc: Dictionary = {"entity_id": 7004, "npc": true, "map_id": "rom-map-4-3", "x": 8, "y": 4}
	...
	if not (world.get("entities") as Dictionary).has("7004"):
		...
		quit(1)
		return
	world.set("story_objects_by_map", {"rom-map-4-3": [{"x": 8, "y": 4, "hide_flag_id": 40}]})
	...
	if not (world.get("entities") as Dictionary).has("7004"):
		...
		quit(1)
		return
	world.set("removed_npc_entities", {"rom-map-4-3": {"7004": true}})
	world.set("npc_entity_maps", {"7004": "rom-map-4-3"})
